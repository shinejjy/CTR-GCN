...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.spdmodel = SPDModel(self.graph, num_point)
...
class SPDModel(nn.Module):
    def __init__(self, graph, num_point=25):
        ...
        if num_point == 25:
            self.model = nn.Sequential(
                ...
                SPDCov2d(6, 6, kernel_size=5, stride=2),
                ReEig(),
                SPDCov2d(6, 6, kernel_size=3, stride=3),
                ReEig(),
                LogEig()
            )
        else:
            self.model = nn.Sequential(
                SPDCov2d(1, 6, kernel_size=5, stride=3),
                ReEig(),
    ...
    def forward(self, device):
        ...
        x = torch.cov(x.T)
        x = x + 1e-8 * torch.eye(N*N, device=device)
        x = x.unsqueeze(0).unsqueeze(0)  # 1, 1, 625, 625
...
class GCN_TCN(nn.Module):
    ...
    def forward(self, x, spd_A=None):
        ...
        if not self.abeta:
            log['pram']['a_beta'] = beta.detach()
        if not self.aalpha:
            log['pram']['a_gamma'] = gamma.detach()
