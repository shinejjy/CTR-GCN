...
from utils.Geom import Embedding_SPD
...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, A3, residual=True, alpha=1, num_heads=8, num_subsets=3):
        super(unit_gcn, self).__init__()
        # (4, 3, 64, 25)
        self.out_c = out_channels
        self.in_c = in_channels
        self.num_heads = num_heads if in_channels > num_heads else 1
        ...
        print(A.shape)
        self.fc1 = nn.Parameter(
            torch.stack([torch.stack([torch.eye(A.shape[-1]) for _ in range(self.num_heads)], dim=0) for _ in
                         range(num_subsets)],
                        dim=0), requires_grad=True)

        self.fc2 = nn.ModuleList(
            [nn.Conv2d(in_channels, out_channels, 1, groups=self.num_heads) for _ in range(num_subsets)])
        ...
        if alpha is None:
            self.alpha = nn.Parameter(torch.ones(num_subsets), requires_grad=True)
        else:
            self.alpha = [alpha for _ in range(num_subsets)] if not isinstance(alpha, list) else alpha

        self.rpe = nn.Parameter(torch.zeros((num_subsets, self.num_heads, int(self.A.max() + 1))))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_subsets):
            ...
            w1 = w1 + pos_emb[i] / self.L2_norm(pos_emb[i]) * self.alpha[i]
...
class Topo(nn.Module):
    def __init__(self, labels, hidden_dim=7, k=3):
        super(Topo, self).__init__()
        self.emb = nn.Conv2d(3, hidden_dim, kernel_size=1)
        self.labels = torch.tensor(labels)
        self.k = k
        self.spd_emb = Embedding_SPD(k, hidden_dim)
        self.model = nn.Sequential(
            BiMap((1, 1, (hidden_dim + 1) * hidden_dim // 2 + self.k, 20)),
            ReEig(),
            BiMap((1, 1, 20, 15)),
            ReEig(),
            BiMap((1, 1, 15, 10)),
            ReEig(),
            LogEig(10, tril=False)
        )

        self.fc = nn.Linear(100, 64)

    def forward(self, x):
        x = self.emb(x)  # N, 16, 64, 25
        N, C, T, V = x.size()
        x = x.mean(2).permute(0, 2, 1)
        # 根据 labels 将 x 分割为不同的身体部分
        body_parts = []
        for part_idx in range(self.labels.max() + 1):
            part_indices = torch.nonzero(self.labels == part_idx, as_tuple=False).squeeze()
            if part_indices.numel() > 0:
                part_data = x[:, part_indices, :]
        ...
        for part in body_parts:
            cov = batch_cov(part)
            tr = torch.sum(torch.diagonal(cov, dim1=-2, dim2=-1), dim=-1).unsqueeze(-1).unsqueeze(-1)
            cov = cov / tr
            cov = cov + 1e-5 * torch.eye(C, device=x.get_device())
            cov_matrices.append(cov)
        cov_matrices = torch.stack(cov_matrices, dim=1).unsqueeze(1).unsqueeze(1)
        # N, 1, 1, 6, C, C
        C = C * (C + 1) // 2 + self.k
        x = self.spd_emb(cov_matrices).view(N, 1, C, C)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.partition_label = self.graph.partition_label
        ...
        self.t0 = TopoTrans(out_dim=3)

        self.t1 = TopoTrans(out_dim=64)
        self.t2 = TopoTrans(out_dim=64)
        self.t3 = TopoTrans(out_dim=64)
        self.t4 = TopoTrans(out_dim=64)

        self.t5 = TopoTrans(out_dim=128)
        self.t6 = TopoTrans(out_dim=128)
        self.t7 = TopoTrans(out_dim=128)

        self.t8 = TopoTrans(out_dim=256)
        self.t9 = TopoTrans(out_dim=256)
        self.topo = Topo(self.partition_label, k=3, hidden_dim=7)
    ...
    def forward(self, x, spd_A=None):
        ...
        if self.alpha is None:
            log['pram'] = {
                'a_alpha1': self.l10.gcn1.alpha[0].detach(),
                'a_alpha2': self.l10.gcn1.alpha[1].detach(),
                'a_alpha3': self.l10.gcn1.alpha[2].detach()
            }
