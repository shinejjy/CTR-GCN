...
class unit_gcn(nn.Module):
    ...
    def forward(self, x):
        ...
        # 直接相加？
        for i in range(self.num_subset):
            A_at.append(self.convs1[i](x, A3[i], A6, self.alpha))
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x):
        # (4, 3, 64, 25)
        y, A3, A6, A_fn, alpha = self.gcn1(x)
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.t1 = TopoTrans(out_dim=60)
        self.t2 = TopoTrans(out_dim=60)
        self.t3 = TopoTrans(out_dim=60)
        self.t4 = TopoTrans(out_dim=60)

        self.t5 = TopoTrans(out_dim=120)
        self.t6 = TopoTrans(out_dim=120)
        self.t7 = TopoTrans(out_dim=120)

        self.t8 = TopoTrans(out_dim=240)
        self.t9 = TopoTrans(out_dim=240)
        self.topo = Topo()
    ...
    def forward(self, x, spd_A=None):
        ...
        x = self.l1(x + self.t0(a))[0]  # (N*M, 60 , 64, 25)
        x = self.l2(x + self.t1(a))[0]  # (N*M, 60, 64, 25)
        x = self.l3(x + self.t2(a))[0]  # (N*M, 60, 64, 25)
        x = self.l4(x + self.t3(a))[0]  # (N*M, 60, 64, 25)

        x = self.l5(x + self.t4(a))[0]  # (N*M, 120, 32, 25)
        x = self.l6(x + self.t5(a))[0]  # (N*M, 120, 32, 25)
        x = self.l7(x + self.t6(a))[0]  # (N*M, 120, 32, 25)

        x = self.l8(x + self.t7(a))[0]  # (N*M, 240, 16, 25)
        x = self.l9(x + self.t8(a))[0]  # (N*M, 240, 16, 25)
        x, A3, A6, A_fn, alpha = self.l10(x + self.t9(a))  # (N*M, 240, 16, 25)
