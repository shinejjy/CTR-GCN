...
class TemporalConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, dilation=1, dropout=0.2):
        super(TemporalConv, self).__init__()
        pad1 = ((kernel_size - 1) * dilation) // 1
        self.conv1 = nn.Conv2d(
            ...
            stride=(1, 1),
            dilation=(dilation, 1))
        self.chomp1 = Chomp(pad1)
        self.relu1 = nn.ReLU(inplace=True)
        self.dropout1 = nn.Dropout(dropout)

        pad2 = ((kernel_size - 1) * dilation) // stride
        self.conv2 = nn.Conv2d(
            ...
            stride=(stride, 1),
            dilation=(dilation, 1))
        self.chomp2 = Chomp(pad2) if stride == 1 else lambda x: x
