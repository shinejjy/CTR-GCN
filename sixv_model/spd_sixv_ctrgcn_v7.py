...
def partPoolingLoss(x, partition_label):
    B, C, T, V = x.shape
    P = max(partition_label) + 1

    # 初始化池化后的结果
    partly_x = torch.zeros(B, C, P, device=x.device)

    # 按照身体部分分组，并进行池化操作
    for partition_idx in range(P):
        partition_indices = [i for i, label in enumerate(partition_label) if label == partition_idx]
        if partition_indices:
            partly_x[:, :, partition_idx] = x[:, :, :, partition_indices].mean(-1).mean(2)

    distances = torch.norm(partly_x.unsqueeze(2) - partly_x.unsqueeze(3), dim=1)
    max_distance, _ = torch.max(F.softmax(distances.mean(1), dim=1), dim=1)
...
class Model(nn.Module):
    ...
    def forward(self, x, spd_A=None):
        ...
        part_pooling_loss = partPoolingLoss(x, self.partition_label)
        ...
        return self.fc(x), part_pooling_loss, log
