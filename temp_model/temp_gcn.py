...
class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, len_temp, stride=1, residual=True, adaptive=True):
        super(TCN_GCN_unit, self).__init__()
        # (4, 3, 64, 25) , out=64
        self.gcn1 = unit_gcn(in_channels, out_channels, A, adaptive=adaptive)
        self.tcn1 = nn.Sequential(
            unit_tcn(out_channels, out_channels, kernel_size=1, stride=stride),
            Temporal_GCN(out_channels, out_channels, len_temp=len_temp)
        )
        self.relu = nn.ReLU(inplace=True)
        if not residual:
            self.residual = lambda x: 0
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        self.l5 = TCN_GCN_unit(base_channel, base_channel*2, A, base_temp_length // 2, stride=2, adaptive=adaptive)
        ...
        self.l8 = TCN_GCN_unit(base_channel*2, base_channel*4, A, base_temp_length // 4, stride=2, adaptive=adaptive)
...
class Temporal_GCN(nn.Module):
    def __init__(self, in_channel, out_channel, len_temp, rel_channel=4, n_head=4, residual=False, learnable=False):
        super(Temporal_GCN, self).__init__()
        assert in_channel % rel_channel == 0 and out_channel % n_head == 0
        self.n_head = n_head
        ...
        self.bns = nn.ModuleList([nn.BatchNorm2d(out_channel // n_head) for _ in range(n_head)])
        self.relu = nn.ReLU(inplace=True)
        if learnable:
    ...
    def forward(self, x):
        ...
        for i in range(self.n_head):
            temporal_A = self.alpha * multi_At[i] + A
            z = torch.einsum('btg,bctv->bcgv', temporal_A, x)
            z = self.bns[i](self.fcs[i](z))
            branch_out.append(z)
