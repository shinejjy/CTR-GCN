...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward
            output, part_pooling_loss, log = self.model(data)

            classification_loss = self.loss(output, label)
            part_pooling_loss = 1.0 - part_pooling_loss.mean(0)
            loss = classification_loss + part_pooling_loss
            ...
            self.train_writer.add_scalar('classification_loss', loss.data.item(), self.global_step)
            self.train_writer.add_scalar('part_pooling_loss', part_pooling_loss.data.item(), self.global_step)
