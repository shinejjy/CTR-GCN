...
class unit_gcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, A_all, coff_embedding=4, residual=True):
        ...
        self.A_all = nn.Parameter(torch.from_numpy(A_all.astype(np.float32)).repeat(self.num_subset, 1, 1),
                                  requires_grad=True)
    ...
    def forward(self, x):
        ...
        A_all = self.A_all.cuda(x.get_device())

        A_at = []  # 6, 64, 25, 25
        # 直接相加？
        for i in range(self.num_subset):
            A_at.append(self.convs1[i](x))

        A_at = torch.stack(A_at, 0)

        A_at = A_at * self.alpha + A.unsqueeze(1).unsqueeze(1) + self.beta * A_all.unsqueeze(1).unsqueeze(1)
        ...
        return y, A, A_all, A_at[:, 0, 0, :, :].squeeze(1).squeeze(1)


class TCN_GCN_unit(nn.Module):
    def __init__(self, in_channels, out_channels, A, A_all, stride=1, residual=True, kernel_size=5,
                 dilations=[1, 2]):
        super(TCN_GCN_unit, self).__init__()
        # (4, 3, 64, 25) , out=64
        self.gcn1 = unit_gcn(in_channels, out_channels, A, A_all)
    ...
    def forward(self, x):
        # (4, 3, 64, 25)
        y, A, A_all, A_at = self.gcn1(x)
        y = self.relu(self.tcn1(y) + self.residual(x))
        return y, A, A_all, A_at
...
class Stage2(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, in_channels=3,
        ...
        base_channel = 64
        # (4, 3, 64, 25)
        self.l1 = TCN_GCN_unit(in_channels, base_channel, A, A_all, residual=False)
        self.l2 = TCN_GCN_unit(base_channel, base_channel, A, A_all)
        self.l3 = TCN_GCN_unit(base_channel, base_channel, A, A_all)
        self.l4 = TCN_GCN_unit(base_channel, base_channel, A, A_all)
        self.l5 = TCN_GCN_unit(base_channel, base_channel * 2, A, A_all, stride=2)
        self.l6 = TCN_GCN_unit(base_channel * 2, base_channel * 2, A, A_all)
        self.l7 = TCN_GCN_unit(base_channel * 2, base_channel * 2, A, A_all)
        self.l8 = TCN_GCN_unit(base_channel * 2, base_channel * 4, A, A_all, stride=2)
        self.l9 = TCN_GCN_unit(base_channel * 4, base_channel * 4, A, A_all)
        self.l10 = TCN_GCN_unit(base_channel * 4, base_channel * 4, A, A_all)
    ...
    def forward(self, x):
        ...
        x = self.l4(x)[0]  # (N*M, 64, 64, 25)
        # x2 = x
        x = self.l5(x)[0]  # (N*M, 128, 32, 25)
        x = self.l6(x)[0]  # (N*M, 128, 32, 25)
        x = self.l7(x)[0]  # (N*M, 128, 32, 25)
        # x3 = x
        x = self.l8(x)[0]  # (N*M, 256, 16, 25)
        x = self.l9(x)[0]  # (N*M, 256, 16, 25)
        x, A, A_all, A_at = self.l10(x)  # (N*M, 256, 16, 25)
