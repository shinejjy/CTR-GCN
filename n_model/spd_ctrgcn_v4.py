...
class unit_gcn(nn.Module):
    ...
    def forward(self, x, A_spd):
        ...
        A_fn = A_at + self.gamma * A_se.unsqueeze(1).unsqueeze(1) + A_spd
        ...
        return y, A_spd.detach(), A.detach(), A_fn[:, 0, 0, :, :].squeeze(1).squeeze(1).detach(), self.alpha.detach(), self.gamma.detach()
...
class TCN_GCN_unit(nn.Module):
    ...
    def forward(self, x, spd_A):
        # (4, 3, 64, 25)
        y, A_spd, A, A_fn, alpha, gamma = self.gcn1(x, spd_A)
        y = self.relu(self.tcn1(y) + self.residual(x))
        return y, A_spd, A, A_fn, alpha, gamma
...
class Stage2(nn.Module):
    ...
    def forward(self, x, spd_A):
        ...
        x, A_spd, A, A_fn, alpha, gamma = self.l10(x, spd_A)  # (N*M, 256, 16, 25)
        ...
        return [self.fc(x), A_spd, A, A_fn, alpha, gamma]
